...
- **`meeting-summaries/`** — Meeting Summary list redesigned with `--dash-*` design system (Phase 28), teal accent. Orchestrator (`meeting-summary-list.tsx`) with saved-view tabs (All / Mine / This week / Needs summary / Pinned / Archived), account filter pill (click a group header), and view-mode segment (Table / Gallery / Timeline). Sub-components: `meetings-header.tsx` (hero + 4-card stats strip incl. 8-week SVG sparkline), `meetings-toolbar.tsx`, `meetings-view-tabs.tsx`, `meetings-subtoolbar.tsx`, `meetings-table.tsx` (account-grouped, 42×42 date tile, pin indicator, first-sentence summary), `meetings-gallery.tsx` (account-grouped card grid), `meetings-timeline.tsx` (Mon–Sun week grid centered on today + off-week overflow), `meetings-quick-add.tsx` (sticky inline title + date picker row), `meetings-bulk-bar.tsx` (teal Spawn action items + Pin/Unpin + Archive + Delete), `labels.ts` (pinned detection, avatar colors, date helpers). `⌘⇧M` opens new summary. Detail / form / delete dialogs unchanged. AI extraction dialog (`extract-action-items-dialog.tsx`) routes the bulk "Spawn action items" flow
- **`ideas/`** — Idea list redesigned with `--dash-*` design system (Phase 28), warm yellow accent, "sketchbook" capture-first UX. Orchestrator (`idea-list.tsx`) with saved-view tabs (All / Mine / New this week / High potential / Archived), CategoryStrip in the hero for click-to-filter, and view-mode segment (Table / Gallery / Kanban). Sub-components: `ideas-header.tsx` (hero + stats + CategoryStrip), `ideas-toolbar.tsx`, `ideas-view-tabs.tsx`, `ideas-subtoolbar.tsx`, `ideas-table.tsx` (category-grouped, bulb row tile, hover Promote/Edit/Delete), `ideas-gallery.tsx` (category-grouped card grid), `ideas-kanban.tsx` (priority columns — substitutes for stage status, since no stage field exists), `ideas-quick-add.tsx` (sticky inline capture row), `ideas-bulk-bar.tsx` (yellow Promote + Archive + Delete), `capture-composer.tsx` (persistent floating composer, `⌘⇧I` focus, `⌘↵` capture, minimizable FAB, localStorage draft autosave), `promote-dialog.tsx` (preview + bulk promote to Work action items, optional archive-after-promote), `labels.ts` (category tints/dots/order, priority pills, high-potential set). Detail / form / delete dialogs unchanged
...
- **`copilot-chat-panel.tsx`** — In-app Copilot Studio chat panel branded as **Compass** (subtitle: "Assisting you in navigating My Work"). Purple→pink gradient floating button (bottom-right, Lucide `Compass` icon) opens a 96-column chat panel with the same gradient header, Compass avatar, message thread, Reset action, and disabled-while-sending input. Calls `MicrosoftCopilotStudioService.ExecuteCopilotAsyncV2()` against `cr7d7_agent` via the `shared_microsoftcopilotstudio` connector — connection ID `27ca174091a844f8bd949ad8ff00f5b9`. Conversation ID is held in a ref so follow-up turns thread correctly; Reset clears the thread and the ID. The popup-only `copilot-chat.tsx` from the commercial repo was not ported
...
- `use-meeting-summaries.ts` — TanStack Query hooks wrapping `Tdvsp_meetingsummariesService` (CRUD + cache invalidation). Exports `useMeetingSummaries` (active-only, statecode filter) and `useAllMeetingSummaries` (no filter — powers the Archived tab from a single client-side cache)
- `use-ideas.ts` — TanStack Query hooks wrapping `Tdvsp_ideasService` (CRUD + cache invalidation). Exports `useIdeas` (active-only) and `useAllIdeas` (no filter — powers the Archived tab)
...
| Ideas list redesign (Phase 28) | `--dash-*` design system, warm yellow accent. Hero with dynamic CategoryStrip. Three view modes: table (grouped by category), gallery (category card grid), kanban (priority columns substituting for the brief's stage status since no stage field exists in Dataverse). Persistent floating `CaptureComposer` (⌘⇧I) with localStorage draft autosave. Bulk bar with Promote-to-Action-Items (copies fields, optionally archives idea; no schema-level back-link since action items have no idea lookup yet). Archived tab driven by `useAllIdeas` + `statecode=1`. Saved views: All / Mine / New this week / High potential / Archived. Schema gaps noted in HOW-I-WAS-BUILT Phase 28 |
| Meetings list redesign (Phase 28) | `--dash-*` design system, teal accent, institutional feel (distinct from Ideas' sketchbook). Hero with 4-card stats strip incl. pure-SVG 8-week cadence sparkline. Three view modes: table (grouped by account with 42×42 date tile — teal future, muted past), gallery (account card grid), timeline (Mon–Sun week grid centered on today, today column outlined + haloed, off-week overflow list). Spawn-action-items bulk action routes into the existing `ExtractActionItemsDialog` (Azure OpenAI). `⌘⇧M` opens new summary. Type/outcome/attendees/tags/transcriptUrl dropped from the brief since fields don't exist on `tdvsp_MeetingSummary`; "Needs summary" saved view substitutes for "Has open follow-ups" |
| Ideas archive = `statecode=1` | Without a separate stage field (new/validating/planned) on `tdvsp_Idea`, archived is modeled as `statecode=1` via `useUpdateIdea`. Promote flow optionally archives the idea to signal "promoted." If a future schema adds stage + a `tdvsp_Idea` lookup on `tdvsp_ActionItem`, the Promoted saved view and stage-kanban can be restored without rewriting the orchestrator |
